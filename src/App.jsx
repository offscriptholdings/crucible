      ...
      return (
        <div data-testid="cockpit" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.18fr 1fr 1fr', gap: 16, alignItems: 'start' }}>
            <div>{/* CalendarPanel — MTC-306 */}</div>
            <div>{/* LoopsPanel + TasksMini — MTC-306/307 */}</div>
            <div>{/* ProtocolsPanel + PeoplePanel — MTC-306 */}</div>
          </div>
        </div>
      )
    }
    return (
      <div data-testid="cockpit" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.08fr 1fr', gap: 16, alignItems: 'start' }}>
          <div>{/* CalendarPanel + PeoplePanel — MTC-306 */}</div>
          <div>{/* LoopsPanel + TasksMini + ProtocolsPanel — MTC-306/307 */}</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', display: 'flex', fontFamily: 'var(--sans)' }}>
      ...
      <main className="cx-scroll" style={{ flex: 1, padding: padMain, paddingBottom: padMain + 78 }}>
        {tab === 'home' ? cockpit() : <div style={{ maxWidth: bp === 'ipad' ? 1180 : 980 }}>{surface()}</div>}
      </main>
